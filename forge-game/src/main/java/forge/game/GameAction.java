...
public class GameAction {
    ...
    // CR 613.8 dependency detection is a battlefield filter per pair of statics in a
    // layer. A pass is idempotent, so its order holds for the next pass while the game
    // state and the layer's statics are the same. An LKI pass without an entry of its
    // own takes the real pass's order; an LKI copy keeps the ids of its statics.
